...
awsDiscovery() {
        \$AWSCLI ec2 describe-instances  --filters Name=tag:aws:autoscaling:groupName,Values=petclinic-prod-asg \\
...
instanceUpdate() {
    sleep 30
    ansible-playbook /etc/ansible/prod-env-playbook.yml --extra-vars "ansible_python_interpreter=/usr/bin/python3.9"
    sleep 30
}
